    ...
    <GuestLayout>
        ...
        <PremiumCard padding="p-8">
            <form @submit.prevent="submit" class="flex flex-col gap-6">
                ...
                <div class="flex flex-col gap-2">
                    <label for="name" class="font-mono text-[10px] uppercase tracking-widest text-muted">Nama Lengkap</label>
                    <input id="name" type="text" v-model="form.name" required autofocus class="w-full border-0 border-b border-border-subtle bg-transparent py-2 text-base text-primary placeholder:text-muted/30 focus:border-primary focus:outline-none focus:ring-0 transition-colors rounded-none px-0" placeholder="cth. Alex Morgan" />
                ...
                <div class="flex flex-col gap-2">
                    <label for="password_confirmation" class="font-mono text-[10px] uppercase tracking-widest text-muted">Konfirmasi Kata Sandi</label>
                ...
                <div class="pt-4">
                    <PremiumButton type="submit" class="w-full" :class="{ 'opacity-50 pointer-events-none': form.processing }">Buat Akun</PremiumButton>
                </div>
            </form>
        </PremiumCard>
        ...
        <div class="mt-8 text-center">
            <p class="text-sm text-muted">
                Sudah punya akun? <Link :href="route('login')" class="text-primary font-medium hover:underline underline-offset-4 decoration-border-subtle hover:decoration-primary transition-all">Masuk</Link>
            </p>
        </div>
    </GuestLayout>
</template>